...
@Composable private fun AuthScreen(api:ApiClient,onSignedIn:(TttUser)->Unit){
 ...
 fun submit(){loading=true;error=null;scope.launch{val r=when{mode=="signup"->api.signup(email,password,username,display);mode=="otp"&&!sent->{val x=api.startOtp(email);if(x.error==null)sent=true;x};mode=="otp"->api.verifyOtp(email,otp);else->api.passwordLogin(email,password)};if(r.value!=null)onSignedIn(r.value);error=r.error;loading=false}}
 Surface(Modifier.fillMaxSize()){Column(Modifier.fillMaxSize().padding(28.dp),verticalArrangement=Arrangement.Center){
  ...
  Field(email,{email=it},"Email",KeyboardType.Email);if(mode!="otp"){Spacer(Modifier.height(10.dp));Field(password,{password=it},"Password",KeyboardType.Password,true)};if(mode=="otp"&&sent){Spacer(Modifier.height(10.dp));Field(otp,{otp=it},"6-digit code",KeyboardType.Number)}
  error?.let{Text(it,color=MaterialTheme.colorScheme.error,modifier=Modifier.padding(top=8.dp))};Spacer(Modifier.height(14.dp))
  Button(::submit,enabled=!loading,modifier=Modifier.fillMaxWidth().height(52.dp)){if(loading)CircularProgressIndicator(Modifier.size(20.dp),color=MaterialTheme.colorScheme.onPrimary)else Text(if(mode=="otp"&&sent)"Verify code"else if(mode=="signup")"Create account"else if(mode=="otp")"Send code"else"Sign in")}
  TextButton(onClick={mode=when(mode){"login"->"otp";"otp"->"signup";else->"login"};sent=false;error=null},modifier=Modifier.fillMaxWidth()){Text(when(mode){"login"->"Use email OTP";"otp"->"Create a new account";else->"Already have an account? Sign in"})}
 }}
}
