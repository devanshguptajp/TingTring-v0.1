...
class ApiClient(private val baseUrl:String,private val session:SessionStore){
 ...
 suspend fun updateProfile(username:String,displayName:String):ApiResult<TttUser>{
  val r=request("PATCH","/api/v1/profile",JSONObject().apply{put("username",username);put("display_name",displayName)},true)
  if(r.value==null)return ApiResult(error=r.error)
  return ApiResult(user(r.value.getJSONObject("user")))
 }
 ...
 suspend fun endCall(callId:String):ApiResult<Unit>{val r=request("POST","/api/v1/calls/"+callId+"/end",auth=true);return if(r.error==null)ApiResult(Unit)else ApiResult(error=r.error)}
 ...
 suspend fun ownerSearchUsers(q:String):ApiResult<List<TttUser>>{val r=request("GET","/api/v1/owner/users/search?q="+URLEncoder.encode(q,"UTF-8"),auth=true);if(r.value==null)return ApiResult(error=r.error);val a=r.value.optJSONArray("results")?:JSONArray();return ApiResult((0 until a.length()).map{user(a.getJSONObject(it))})}
 suspend fun ownerChangeTttId(userId:String,newId:String):ApiResult<Pair<String,String>>{val r=request("POST","/api/v1/owner/users/"+userId+"/ttt-id",JSONObject().apply{put("ttt_user_id",newId)},true);if(r.value==null)return ApiResult(error=r.error);return ApiResult(r.value.optString("old_ttt_user_id") to r.value.optString("ttt_user_id"))}
